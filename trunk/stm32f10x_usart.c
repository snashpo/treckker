...
/*
 * Deinitializes the USARTx peripheral registers to their
...
void USART_Init(USART_TypeDef* USARTx, USART_InitTypeDef* USART_InitStruct)
{
  ...
  /* 
   * The hardware flow control is available only for 
   * USART1, USART2 and USART3 
   */
  assert_param(IS_USART_PERIPH_HFC(USARTx, 
		  USART_InitStruct->USART_HardwareFlowControl));
  ...
  } else { /* if (usartxbase == USART1_BASE) */
		    ...
		    FunctionalState NewState)
{
  ...
  assert_param(IS_USART_PERIPH_IT(USARTx, USART_IT)); 
  assert_param(IS_FUNCTIONAL_STATE(NewState));
  usartxbase = (*(uint32_t*)&(USARTx));
...
FlagStatus USART_GetFlagStatus(USART_TypeDef* USARTx, uint16_t USART_FLAG)
{
  ...
  assert_param(IS_USART_PERIPH_FLAG(USARTx, USART_FLAG)); 

  if ((USARTx->SR & USART_FLAG) != (uint16_t)RESET) {
    bitstatus = SET;
  } else { /* if ((USARTx->SR & USART_FLAG) != (uint16_t)RESET) */
    bitstatus = RESET;
  } /* if ((USARTx->SR & USART_FLAG) != (uint16_t)RESET) */
...
ITStatus USART_GetITStatus(USART_TypeDef* USARTx, uint16_t USART_IT)
{
  ...
  if ((itmask != (uint16_t)RESET)&&(bitpos != (uint16_t)RESET)) {
    bitstatus = SET;
  } else { /* if ((itmask != (uint16_t)RESET)&&(bitpos != (uint16_t)RESET)) */
    bitstatus = RESET;
  } /* if ((itmask != (uint16_t)RESET)&&(bitpos != (uint16_t)RESET)) */
