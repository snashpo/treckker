...
FLASH_Status FLASH_EraseOptionBytes(void)
{
  ...
  if (status == FLASH_COMPLETE) {
    ...
    /* 
     * if the previous operation is completed, 
...
FlagStatus FLASH_GetReadOutProtectionStatus(void)
{
  ...
  if ((FLASH->OBR & RDPRT_Mask) != (uint32_t)RESET) {
    readoutstatus = SET;
  } else { /* if ((FLASH->OBR & RDPRT_Mask) != (uint32_t)RESET) */
    readoutstatus = RESET;
  } /* if ((FLASH->OBR & RDPRT_Mask) != (uint32_t)RESET) */
...
FlagStatus FLASH_GetPrefetchBufferStatus(void)
{
  ...
  if ((FLASH->ACR & ACR_PRFTBS_Mask) != (uint32_t)RESET) {
    bitstatus = SET;
  } else { /* if ((FLASH->ACR & ACR_PRFTBS_Mask) != (uint32_t)RESET) */
    bitstatus = RESET;
  } /* if ((FLASH->ACR & ACR_PRFTBS_Mask) != (uint32_t)RESET) */
...
FlagStatus FLASH_GetFlagStatus(uint16_t FLASH_FLAG)
{
  ...
  if (FLASH_FLAG == FLASH_FLAG_OPTERR) {
    ...
    if ((FLASH->OBR & FLASH_FLAG_OPTERR) != (uint32_t)RESET) {
      bitstatus = SET;
    } else { /* if ((FLASH->OBR & FLASH_FLAG_OPTERR) != (uint32_t)RESET) */
      bitstatus = RESET;
    } /* if ((FLASH->OBR & FLASH_FLAG_OPTERR) != (uint32_t)RESET) */
    ...
    if ((FLASH->SR & FLASH_FLAG) != (uint32_t)RESET) {
      bitstatus = SET;
    } else { /* if ((FLASH->SR & FLASH_FLAG) != (uint32_t)RESET) */
      bitstatus = RESET;
    } /* if ((FLASH->SR & FLASH_FLAG) != (uint32_t)RESET) */
...
FLASH_Status FLASH_GetStatus(void)
{
  ...
  } else { /* if ((FLASH->SR & FLASH_FLAG_BSY) == FLASH_FLAG_BSY) */
  
    if (FLASH->SR & FLASH_FLAG_PGERR) { 
      flashstatus = FLASH_ERROR_PG;
    } else { /* if (FLASH->SR & FLASH_FLAG_PGERR) */

      if (FLASH->SR & FLASH_FLAG_WRPRTERR) {
        flashstatus = FLASH_ERROR_WRP;
      } else { /* if (FLASH->SR & FLASH_FLAG_WRPRTERR) */
