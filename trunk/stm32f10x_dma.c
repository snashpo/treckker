	      ...
	      DMA_InitTypeDef* DMA_InitStruct)
{
  ...
  /* 
   * Configure DMAy Channelx: data transfer, data size, 
   * priority level and mode 
   */
...
FlagStatus DMA_GetFlagStatus(uint32_t DMA_FLAG)
{
  ...
  if ((DMA_FLAG & FLAG_Mask) != (uint32_t)RESET) {
    ...
    tmpreg = DMA2->ISR ;
  } else { /* if ((DMA_FLAG & FLAG_Mask) != (uint32_t)RESET) */
  ...
  if ((tmpreg & DMA_FLAG) != (uint32_t)RESET) {
    /* DMA_FLAG is set */
    bitstatus = SET;
  } else { /* if ((tmpreg & DMA_FLAG) != (uint32_t)RESET) */
    /* DMA_FLAG is reset */
    bitstatus = RESET;
  } /* if ((tmpreg & DMA_FLAG) != (uint32_t)RESET) */
...
ITStatus DMA_GetITStatus(uint32_t DMA_IT)
{
  ...
  if ((DMA_IT & FLAG_Mask) != (uint32_t)RESET) {
    /* Get DMA2 ISR register value */
    tmpreg = DMA2->ISR ;
  } else { /* if ((DMA_IT & FLAG_Mask) != (uint32_t)RESET) */
    /* Get DMA1 ISR register value */
    tmpreg = DMA1->ISR ;
  } /* if ((DMA_IT & FLAG_Mask) != (uint32_t)RESET) */
  ...
  if ((tmpreg & DMA_IT) != (uint32_t)RESET) {
    /* DMA_IT is set */
    bitstatus = SET;
  } else { /* if ((tmpreg & DMA_IT) != (uint32_t)RESET) */
    /* DMA_IT is reset */
    bitstatus = RESET;
  } /* if ((tmpreg & DMA_IT) != (uint32_t)RESET) */
