  ...
  if (EXTI_InitStruct->EXTI_LineCmd != DISABLE) {
    ...
    *(__IO uint32_t *)(EXTI_BASE + (uint32_t)EXTI_InitStruct->EXTI_Mode)|= 
    ...
    if (EXTI_InitStruct->EXTI_Trigger == EXTI_Trigger_Rising_Falling) {
      ...
      EXTI->RTSR |= EXTI_InitStruct->EXTI_Line;
      EXTI->FTSR |= EXTI_InitStruct->EXTI_Line;
    } else { /* if (EXTI_InitStruct->EXTI_Trigger == EXTI_Trigger_... */

      *(__IO uint32_t *)(EXTI_BASE + (uint32_t)EXTI_InitStruct->EXTI_Trigger)|=
	EXTI_InitStruct->EXTI_Line;
    } /* if (EXTI_InitStruct->EXTI_Trigger == EXTI_Trigger_... */

  } else { /* if (EXTI_InitStruct->EXTI_LineCmd != DISABLE) */
...
FlagStatus EXTI_GetFlagStatus(uint32_t EXTI_Line)
{
  ...
  if ((EXTI->PR & EXTI_Line) != (uint32_t)RESET) {
    bitstatus = SET;
  } else { /* if ((EXTI->PR & EXTI_Line) != (uint32_t)RESET) */
    bitstatus = RESET;
  } /* if ((EXTI->PR & EXTI_Line) != (uint32_t)RESET) */
